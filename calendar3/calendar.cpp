...
void Calendar::addAppointment()
{
  ...
  for(pos = 0;
      pos < count && !dayInput.equal(&days[pos]);
      pos++);
  ...
  if(pos == count) // not found
  {
    ...
    for(pos = count - 1;
      pos >= 0 && dayInput.lessThan(&days[pos]); pos--)
        days[pos + 1] = days[pos];

    days[++pos] = dayInput;  // copy the new day into pos + 1
    count++;
  } // if not found

  days[pos].readDay(sub, loc);
  return;
} // addAppointment
